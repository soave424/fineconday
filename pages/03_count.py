...
if access_code == "z733":
    ...
    if data.empty:
        ...
        # Course information dictionary with instructor, code, and location
        course_info = {
        ...
        course_counts_df['코드'] = course_counts_df['강좌명'].apply(lambda x: course_info.get(x, ("", "", "미정"))[1])
        course_counts_df['장소'] = course_counts_df['강좌명'].apply(lambda x: course_info.get(x, ("", "", "미정"))[2])

        # Sorting options
        sort_option = st.radio("정렬 방식 선택", ["인원수별", "시간대별 (강좌 코드 순)"])

        if sort_option == "인원수별":
            # Sort by participant count
            sorted_df = course_counts_df.sort_values(by='신청 인원수', ascending=False).reset_index(drop=True)
            st.title("강좌별 신청 인원수 (인원수별 정렬)")
            st.table(sorted_df[['강좌명', '강사명', '신청 인원수', '코드', '장소']])
        else:
            # Sort by time slot (강좌 코드 순) and group by time slot prefix
            sorted_df = course_counts_df.sort_values(by='코드').reset_index(drop=True)

            st.title("강좌별 신청 인원수 (시간대별 정렬)")
            ...
            for index, row in sorted_df.iterrows():
                ...
                if code_prefix != current_prefix:
                    st.markdown(f"### 선택 강좌 {code_prefix.upper()}")
                    current_prefix = code_prefix
